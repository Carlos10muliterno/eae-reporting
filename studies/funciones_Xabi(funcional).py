...
# busqueda = input("Introduce palabra a buscar: ")
...
def collectData(busqueda):
    ...
    while (count<1500):
        if count == 0:
            request_url     = "https://api.twitter.com/2/tweets/search/recent?query="+ busqueda + "&tweet.fields=" + "created_at,text,public_metrics,lang,geo"+"&max_results=" + str(ntweets) 
            ...
            request_url     = "https://api.twitter.com/2/tweets/search/recent?query="+ busqueda + "&tweet.fields=" + "created_at,text,public_metrics,lang,geo"+ "&max_results=" + str(ntweets) + "&next_token="+ json_response_twitter['meta']['next_token']
            json_response_twitter = connect_to_endpoint_twitter(request_url, headers)
            for tweet in json_response_twitter['data']:    
                list_of_tweets.append(tweet)
            count += ntweets
...
def dataTreatment(data_googlemaps,data_twitter):    
