...
def collectData(busqueda, pk):
    ...
    try:
        ...
        list_countries = ["spain", "USA", "france", "uk", "italy", "portugal", "germany", "canada", "mexico", "brasil",
                          "argentina"]
        list_place_ids = []
        ...
        for country in list_countries:
            ...
            search_url = "https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=" + required_params[
                "input"] + " " + country + "&inputtype=" + required_params["inputtype"] + "&fields=" + required_params[
                             "fields"] + "&key=" + required_params["api_key"]
        ...
        for place_id in list_place_ids:
            ...
            search_url_details = "https://maps.googleapis.com/maps/api/place/details/json?place_id=" + place_id + "&fields=" + \
                                 required_params_details["fields"] + "&key=" + required_params_details["api_key"]
        ...
        while (count < 1500):
            if count == 0:
                request_url = "https://api.twitter.com/2/tweets/search/recent?query=" + busqueda + "&tweet.fields=" + "created_at,text,public_metrics,lang,geo" + "&max_results=" + str(
                    ntweets) + "&expansions=author_id&user.fields=name,username,profile_image_url"
                ...
                request_url = "https://api.twitter.com/2/tweets/search/recent?query=" + busqueda + "&tweet.fields=" + "created_at,text,public_metrics,lang,geo" + "&max_results=" + str(
                    ntweets) + "&expansions=author_id&user.fields=name,username,profile_image_url" + "&next_token=" + \
                              json_response_twitter['meta']['next_token']
                json_response_twitter = connect_to_endpoint_twitter(request_url, headers)
                for tweet in json_response_twitter['data']:
                    data_twitter.append(tweet)
                for author in json_response_twitter['includes']['users']:
        ...
        plt.pie(porcentajes, labels=etiquetas, autopct='%1.2f%%', shadow=True, startangle=90, colors=colors,
                explode=explode)

        plt.title("ANÁLISIS DE LOS SENTIMIENTOS",
        ...
        colors = ["red", "salmon", "mistyrose"]
        ntweets = [percentage_positives * (len(df_twitter)), percentage_negatives * (len(df_twitter)),
                   percentage_neutrals * (len(df_twitter))]
        ...
        # GRÁFICO 3
        fig, ax = plt.subplots()
        plt.title("FRECUENCIA DE PALABRAS EN INGLÉS",
                  fontdict={'family': 'serif',
                            'color': 'darkred',
                            'weight': 'bold',
                            'size': 18})
        ...
        plt.title("FRECUENCIA DE PALABRAS EN ESPAÑOL",
                  fontdict={'family': 'serif',
                            'color': 'darkred',
                            'weight': 'bold',
                            'size': 18})
        ...
        plt.title("RETWEET, ME GUSTA Y NÚMERO DE RESPUESTAS",
                  fontdict={'family': 'serif',
                            'color': 'darkred',
                            'weight': 'bold',
                            'size': 18})
        ...
        plt.bar(X - 0.25, medias[0], color=colors[0], width=0.25, label='Retweets')
        plt.bar(X + 0.00, medias[1], color=colors[1], width=0.25, label='Likes')
        plt.bar(X + 0.25, medias[2], color=colors[2], width=0.25, label='Replies')
        plt.xticks(X, ('Positivo', 'Negativo', 'Neutro'))
        ...
        plt.title("MEDIA COMPUESTA POR IDIOMA",
                  fontdict={'family': 'serif',
                            'color': 'darkred',
                            'weight': 'bold',
                            'size': 18})
